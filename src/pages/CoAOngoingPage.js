...
const CoAOngoingPage = ({ data }) => {
  const { notesMarkdown, table } = data.attributes.pageData;
  ...
  return (
    <Card title={data.attributes.title} className="overflow-y-auto">
      <h3 className="text-md mb-2 font-bold">Ongoing Advice</h3>
      <div className="mx-auto mb-4 max-h-full rounded-md bg-gradient-to-r from-blue-100 via-blue-50 p-4">
        <div className="flex flex-col">
          ...
          >
            <p className="text-md mr-auto font-bold capitalize">Description</p>
          ...
          {table?.items?.map((item, idx) => {
            return (
              ...
              >
                <p className="text-md mr-auto capitalize">{item.display}</p>
                ...
                <div className="text-md mr-2 grid w-72 grid-cols-2 gap-2 whitespace-nowrap py-1 px-2 text-center">
                  ...
                  <p className="col-span-1">
                    {item.paidByProduct
                      ? formatMoney(item.paidByProduct, 2, 2, "currency")
                      : "-"}
                  </p>
                </div>
                <div className="text-md grid w-72 grid-cols-2 gap-2 whitespace-nowrap py-1 px-2 text-center">
                  ...
                  <p className="col-span-1">
                    {item.retainedByCar
                      ? formatMoney(item.retainedByCar, 2, 2, "currency")
                      : "-"}
                  </p>
                </div>
              </div>
            ...
            <div className="text-md mr-2 grid w-72 grid-cols-2 gap-2 whitespace-nowrap py-1 px-2 text-center">
              <p className="col-span-1">
                {table?.subTotal?.paidByYou
                  ? formatMoney(table?.subTotal?.paidByYou, 2, 2, "currency")
                  : ""}
              </p>
              <p className="col-span-1">
      ...
      <h3 className="text-md mb-2 font-bold">Note:</h3>

      <ul className="max-w-7xl list-inside list-disc space-y-2">
        {notesMarkdown?.map((p, idx) => (
          <li key={idx}>
            <ReactMarkdown
              className="inline"
              children={p}
