    ...
    <ul className="flex items-center justify-center py-8">
      {payload.map((entry, index) => (
        <li key={`item-${index}`} className="ml-4 flex items-center ">
          ...
          />
          <span className="text-xs text-gray-500">{textMap[entry.value]}</span>
        </li>
      ))}
    </ul>
  );
    ...
    <Card title={data.attributes.title} className="overflow-y-auto">
      ...
      <ResponsiveContainer width="100%" height={500}>
        ...
        >
          ...
          />
          <Bar yAxisId={3} dataKey="income" fill="#74CC6E77" stroke="#74CC6E" />
        </ComposedChart>
      </ResponsiveContainer>
      <div className="mx-auto w-full max-w-4xl">
        <p className="mb-2 text-center text-xs text-gray-400">Filter By Age</p>
        <ReactSlider
